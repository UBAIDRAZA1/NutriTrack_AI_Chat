...
const TrackCalories = () => {
  ...
  useEffect(() => {
    const calculateCalorieTarget = () => {
      ...
      setUserProfile((prev) => ({ ...prev, dailyCalorieTarget: Math.round(target) }));
    };
  ...
  const analyzeHealth = async () => {
    ...
    try {
      setHealthAnalysis(
        `Your health status is good! You've consumed ${caloriesConsumed} kcal out of ${userProfile.dailyCalorieTarget} kcal. You've earned ${points} points. Keep logging low-calorie meals to earn more!`
      );
  ...
  // Share progress on X
  const shareProgress = () => {
    const text = `My daily progress: Consumed ${caloriesConsumed} kcal, Remaining ${Math.max(0, userProfile.dailyCalorieTarget - caloriesConsumed)} kcal! Points: ${points} #Fitness #Health`;
  ...
  if (error) {
    return (
      <div className="container mx-auto p-4 min-h-screen bg-white">
        ...
        <div className="text-red-600 p-3 bg-red-100 rounded-lg animate-pulse text-center">{error}</div>
      </div>
    );
  }
      ...
      <div className="bg-white p-4 rounded-xl shadow-lg mb-6 border border-gray-200 max-w-md mx-auto">
        ...
        {!isEditingProfile ? (
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <p className="col-span-2"><span className="font-medium">Target:</span> {userProfile.dailyCalorieTarget} kcal</p>
            <p><span className="font-medium">Consumed:</span> {caloriesConsumed} kcal</p>
            <p><span className="font-medium">Remaining:</span> {Math.max(0, userProfile.dailyCalorieTarget - caloriesConsumed)} kcal</p>
